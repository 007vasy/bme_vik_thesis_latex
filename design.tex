...
\cite{LeanThinking}
		...
		\caption{To optimise the cleaning workflow, merged the to processes together} 
		\end{figure}
		One have get rid of the resource wasting, fiddling values, and extract the can be usefull columns. After that the graph building values can be selected.
...
explain and reason the validity of the whole method
		...
		Edges truck ID, timestampt warning or exchange part ID
		form that we build a graph and with the given sequence we looking for the truck has similar path after that, from the last node of the sequence calculate possibility paths all of the exhange part ID-s. We display it with avg, min max mean time frame and calced path possibility.