...
\subsection{Calculating RUL (Remaining Useful Life)}
	...
	\begin{enumerate}
		...
		\item{New columns with NA-s was generated to the calculated RULs, based on the count of the tire measurement points and attributes count\footnote{New columns count = tire measurement points $*$ attributes count.}
		\item{Tire abrasion changes are calculated by attributes to the previously generated column}
		\item{Based on the last aggregation window mean abrasion rate, the RUL are calculable\footnote{These results are usable to predict other machines tire abrasion, if the attributes are calculated and aggregated in a similar time window.} .}
