...
\paragraph\noindent
This sections would describe the algoritmization and realization of the previously described process in the design chapter in R language.The reasons are simple, the R is one of the most common and convenient tools to process datasets and interpret them, with a fitted statistical or machine learning model.
%----------------------------------------------------------------------------
\subsection{Cleaning}
%----------------------------------------------------------------------------
		...
		\begin{itemize}
			...
			\item{lubridate:} Also from the the tidyverse meta package, although with a distinct purpose. This package is beneficial to handle time series and time related operation. With the MATLAB export files and time stamp key this is a needed unsavory.
...
\subsection{Calculating RUL (Remaining Useful Life)}
	...
	The hourly aggregated attributes are in a dataframe without the sensory values. A new columns have generated with NA-s and the tire measurements are imported with multiple measurement points. Moreover, inserted to the new columns next to the appropriate hourly time stamp key. When it is done the interpolation should begin to all aggregations points. \footnote{The interpolation was linear. Thus it could be more precise when the interpolation is based on some metascore the elapsed time and travelled distance involved, or with some machine learning algorithm.}
